...
class MapWidget extends StatefulWidget {
  ...
  final bottomDrag;
  MapWidget({this.commonMapType, this.backPark, this.layerType, this.bottomDrag});
...
class _MapWidgetState extends State<MapWidget> {
  ...
  void _switchLayer(String type) {
    _markers.clear();
    switch (type) {
      case 'station': print('初始站点坐标'); break;
      case 'enterprise': _getEnterprise(); break;
      case 'riskSource': _getRiskSource(); break;
      case 'sensitive': _getSensitive(); break;
      case 'wastePoint': _getWastePoint(); break;
    }
  }
    ...
    if(response['code'] == 200) {
      ...
      for (var i = 0; i < border.length; i++) {
        ...
        for (var j = 0; j < itemData.length; j++) {
          List position = GpsUtil.gps84_To_Gcj02(itemData[j][1], itemData[j][0]);
    ...
    return FlutterMap(
      ...
      layers: [
        TileLayerOptions(
          ...
          additionalOptions: {
            'accessToken': 'pk.eyJ1IjoibGVtb25zY2V0IiwiYSI6ImNramk3bGE5eDE3ZDUycm83aXMzajBzZGwifQ.MU0fybBa0127PbYREKzvDA',
            'id':  'mapbox.satellite',
          },
        ),
        PolylineLayerOptions(
          polylines: _polylines.map((item) => (
            Polyline(
              points: item,
              strokeWidth: 2.0,
              color: Color(0XFF3093DA)
            )
          )).toList(),
        ),
  ...
  Marker commonMaker({ double latiude, double longitude, String icon, String markerName, bool selected, Function onTap,}) {
    return Marker(
      height: px(160.0),
      width: px(160.0),
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Text(
              ...
              style: TextStyle(
                fontSize: sp(26.0),
                color: widget.commonMapType ? Color(0XFF45474D) : Color(0XFFFFFFFF),
